            ...
            <div class="auth-content">
                ...
                <div class="auth-container">
                    <h2 style="font-size: xx-large;color:#f5f5f5; font-weight: bolder; margin-bottom:20px;"
                        class="text-center">Bamesa Login</h2>
                    <x-auth-session-status class="mb-4" :status="session('status')" />
                    <form wire:submit="login">
                        <label style="color: #f5f5f5" class="auth-label" for="name">Email: </label>
                        <x-text-input wire:model="form.email" id="email" class="auth-input" type="email" name="email"
                            required autofocus autocomplete="username" />
                        <x-input-error :messages="$errors->get('form.email')" class="mt-2" />
                        <label style="color: #f5f5f5" class="auth-label" for="name">Password: </label>
                        <x-text-input wire:model="form.password" id="password" class="auth-input" type="password"
                        ...
                        <div class="items-center justify-end mt-2">
                            ...
                            <x-primary-button style="color: rgb(17 24 39); text-align:center"
                                class="auth-button bg-white">
                                {{ __('Login') }}
                            </x-primary-button>
                        </div>
                        <div class="link-ctrl   mt-2 ">
                            @if (Route::has('password.request'))
                                <a class=" forgot" href="{{ route('password.request') }}" wire:navigate>
                                    {{ __('Forgot your password?') }}
                                </a>
                            @endif
                            @if (Route::has('register'))
                                <a href="{{ route('register') }}" class="auth-link">Don't have an account? <b>Sign
                                        up</b></a>
                            @endif
                        </div>
